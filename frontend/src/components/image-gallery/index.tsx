      ...
      <div className={clsx({ 'grid grid-cols-2 gap-1': numberOfImages >= 2 }, 'w-full')}>
        {[1, 2, 4].includes(numberOfImages) &&
          images.map((image, index) => (
            <img key={index} src={image} alt="" className="aspect-square w-full object-cover" />
          ))}

        {numberOfImages === 3 &&
          images.map((image, index) => (
            <React.Fragment key={index}>
              {index === 0 && <img src={image} alt="" className="h-full w-full object-cover row-span-2" />}
              {index !== 0 && <img src={image} alt="" className="aspect-square w-full object-cover" />}
            </React.Fragment>
          ))}
        ...
        {numberOfImages > 4 && (
          <>
            {images.slice(0, 3).map((image, index) => (
              <img key={index} src={image} alt="" className="aspect-square w-full object-cover cursor-pointer" />
            ))}
